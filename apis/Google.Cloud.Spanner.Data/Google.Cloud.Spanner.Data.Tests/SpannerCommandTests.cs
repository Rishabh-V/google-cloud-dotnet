...
namespace Google.Cloud.Spanner.Data.Tests
{
    public class SpannerCommandTests
    {
        ...
        public static IEnumerable<object[]> ConfiguredClrTypes()
        {
            ...
            yield return new object[] { "Float64ToFloat", 3.14d, SpannerDbType.Float64, typeof(float) };
        ...
        public void ExecuteReadRequest_ColumnDefaultsToConfiguredClrType(string spannerToClrTypeMapping, object value, SpannerDbType dbType, System.Type clrType)
        {
            var builder = new SpannerConnectionStringBuilder();
            ...
            Value protobufValue = RunReadRequest(dbType, value).Values[0];
            ...
            // Check the values.
            if (dbType == SpannerDbType.Float64)
            {
                Assert.Equal(Convert.ToDouble(value).ToString(), clrValue.ToString());
            }
            else
            {
                // DateTime
                Assert.Equal(DateTime.Parse(value.ToString()), DateTime.Parse(clrValue.ToString()));
            }
        }
        ...
        private ListValue RunReadRequest(SpannerDbType dbType, object value)
        {
